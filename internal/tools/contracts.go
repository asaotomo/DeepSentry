...
var explicitContracts = map[string]ToolContract{
	"task_wait": {Examples: []map[string]string{{"mode": "delay", "timeout_sec": "1"}}, Args: []ArgSpec{
		{Name: "mode", Type: "string", Required: true, Enum: []string{"delay", "file_exists", "file_ready"}},
		{Name: "path", Type: "string", Description: "本机绝对路径"},
		{Name: "timeout_sec", Type: "integer", Description: "1..86400，默认60；delay 的等待时长"},
		{Name: "interval_sec", Type: "integer", Description: "1..60，默认2"},
		{Name: "stable_sec", Type: "integer", Description: "0..3600，默认3"},
		{Name: "expected_size", Type: "integer", Description: "预期总字节数；file_ready 与 sha256 至少一个"},
		{Name: "sha256", Type: "string", Description: "预期 SHA256"},
	}},
	"task_context": {Examples: []map[string]string{{"action": "read"}}, Args: []ArgSpec{
		{Name: "action", Type: "string", Required: true, Enum: []string{"read", "update"}},
		{Name: "goal", Type: "string"}, {Name: "progress", Type: "string"}, {Name: "next_step", Type: "string"}, {Name: "waiting_for", Type: "string"}, {Name: "side_effects", Type: "string"}, {Name: "evidence", Type: "string"},
	}},
	"computer_use": {
		Args: []ArgSpec{
			{Name: "action", Type: "string", Required: true, Description: "status 诊断；observe 截图并附带 macOS 无障碍元素。输入后自动附图，不代表业务成功。stop 中止，resume 恢复，release 释放，wait 只等待后刷新截图。", Enum: []string{"status", "observe", "click", "double_click", "move", "drag", "scroll", "type", "key", "click_element", "set_value", "wait", "stop", "resume", "release"}},
			{Name: "observation_id", Type: "string", Description: "输入必填：当前会话最新截图 id，60 秒内有效；每次输入消费该截图"},
			{Name: "action_id", Type: "string", Description: "输入必填：8..128 字符唯一请求标识。不确定的重试必须沿用原 ID 和原参数，不能换 ID 盲目重放"},
			{Name: "x", Type: "integer", Description: "截图像素 x（从 0 起），不是缩放后的预览坐标。scroll 可与 y 一起指定滚动位置"},
			{Name: "y", Type: "integer", Description: "截图像素 y（从 0 起）"},
			{Name: "to_x", Type: "integer", Description: "拖拽终点截图像素 x"},
			{Name: "to_y", Type: "integer", Description: "拖拽终点截图像素 y"},
			{Name: "button", Type: "string", Description: "默认 left", Enum: []string{"left", "right", "middle"}},
			{Name: "text", Type: "string", Description: "原样输入或 set_value 写入 1..2000 Unicode 字符，不修改剪贴板。不要输入密码/令牌；交给用户完成认证"},
			{Name: "key", Type: "string", Description: "CTRL/ALT/SHIFT/META 加 A-Z、0-9 或 ENTER/TAB/ESCAPE/SPACE/BACKSPACE/DELETE/UP/DOWN/LEFT/RIGHT/HOME/END/PAGEUP/PAGEDOWN，例如 CTRL+A"},
			{Name: "amount", Type: "integer", Description: "滚动 -10..-1 向上，1..10 向下"},
			{Name: "element_index", Type: "integer", Description: "当前截图元素列表中的 index。只在 grounding=element 时使用，换一帧即失效"},
			{Name: "wait_ms", Type: "integer", Description: "wait 的等待毫秒数，200..3000。不注入键鼠"},
		},
		Actions: []ActionSpec{
			{Name: "status"}, {Name: "observe"}, {Name: "stop"}, {Name: "resume"}, {Name: "release"},
			{Name: "wait", Required: []string{"wait_ms"}},
			{Name: "click", Required: []string{"observation_id", "action_id", "x", "y"}},
			{Name: "double_click", Required: []string{"observation_id", "action_id", "x", "y"}},
			{Name: "move", Required: []string{"observation_id", "action_id", "x", "y"}},
			{Name: "drag", Required: []string{"observation_id", "action_id", "x", "y", "to_x", "to_y"}},
			{Name: "scroll", Required: []string{"observation_id", "action_id", "amount"}},
			{Name: "type", Required: []string{"observation_id", "action_id", "text"}},
			{Name: "key", Required: []string{"observation_id", "action_id", "key"}},
			{Name: "click_element", Required: []string{"observation_id", "action_id", "element_index"}},
			{Name: "set_value", Required: []string{"observation_id", "action_id", "element_index", "text"}},
		},
		Examples: []map[string]string{{"action": "status"}, {"action": "observe"}},
	},
	...
	"awd_service_check": {
		Args: []ArgSpec{
			{Name: "targets", Type: "string", Required: true, Description: "HTTP(S) URL 或 host:port；逗号、分号或换行分隔"},
			{Name: "timeout", Type: "integer", Description: "每个目标超时秒数，默认 3，最大 15"},
			{Name: "concurrency", Type: "integer", Description: "并发数，默认 10，最大 32"},
			{Name: "expected_status", Type: "integer", Description: "可选预期 HTTP 状态码；不匹配标记 WARN"},
			{Name: "contains", Type: "string", Description: "可选响应正文关键文本；只读取前 128 KiB"},
		},
		Examples: []map[string]string{{"targets": "http://127.0.0.1:8080,127.0.0.1:22", "timeout": "3", "concurrency": "10"}},
	},
	"fleet_inventory": {
		Args:     []ArgSpec{{Name: "selector", Type: "string", Description: "逗号表示同时满足，| 表示任选；支持 name:/host:/protocol:/tag:，留空或 all 表示全部", Example: "tag:prod,protocol:ssh"}},
		Examples: []map[string]string{{"selector": "all"}, {"selector": "prod,ssh"}},
	},
	"fleet_exec": {
		Args: []ArgSpec{
			{Name: "selector", Type: "string", Description: "逗号表示同时满足，| 表示任选；支持 name:/host:/protocol:/tag:；留空表示 all", Example: "prod,ssh"},
	...
	"fleet_file": {
		Args: []ArgSpec{
			{Name: "selector", Type: "string", Description: "逗号表示同时满足，| 表示任选；留空表示 all"},
			...
			{Name: "local_path", Type: "string", Description: "download/upload 使用的控制端路径；多目标下载自动添加目标后缀以免覆盖"},
			{Name: "concurrency", Type: "integer", Description: "并发数，默认 5，最大 20"},
		},
		Actions: []ActionSpec{
	...
	"http_proxy": {
		Args: []ArgSpec{
			{Name: "action", Type: "string", Description: "代理动作", Required: true, Enum: []string{"start", "list", "stop"}},
			{Name: "listen_host", Type: "string", Description: "监听地址，默认 127.0.0.1"},
			{Name: "listen_port", Type: "integer", Description: "监听端口，start 默认 8080；stop 必填"},
			{Name: "username", Type: "string", Description: "可选认证用户名；必须与 password 同时提供"},
			{Name: "password", Type: "string", Description: "可选认证密码；必须与 username 同时提供"},
			{Name: "allow_lan", Type: "boolean", Description: "是否明确允许监听非回环地址，默认 false"},
		},
		Actions: []ActionSpec{
			{Name: "start", Description: "启动临时 HTTP 代理，供用户程序把流量指过来"},
			{Name: "list", Description: "列出当前代理"},
			{Name: "stop", Description: "按监听端口停止代理", Required: []string{"listen_port"}},
		},
		Examples: []map[string]string{{"action": "list"}, {"action": "start", "listen_host": "127.0.0.1", "listen_port": "8080", "allow_lan": "false"}},
	},
	...
	"schedule_task": {
		Args: []ArgSpec{
			{Name: "action", Type: "string", Description: "定时任务动作", Required: true, Enum: []string{"plan", "add", "list", "show", "edit", "remove", "pause", "cancel", "resume", "reset", "run", "run-due"}},
			{Name: "text", Type: "string", Description: "可选补充。周期任务不要只把整句放在这里"},
			{Name: "task", Type: "string", Description: "要执行的任务正文；创建和 edit 都用它"},
			{Name: "run_at", Type: "string", Description: "单次或定点执行时间，例如 2026-06-27 09:00"},
			{Name: "repeat", Type: "string", Description: "once、daily、weekly、interval，或 每分钟/每小时"},
			{Name: "interval_sec", Type: "string", Description: "周期秒数或 1m/30m/1h/每分钟。每分钟=60，最短 60 秒"},
			{Name: "name", Type: "string", Description: "edit 时的任务名称"},
			...
			{Name: "timeout_sec", Type: "integer", Description: "单次 Agent 时限秒数，默认7200，最大604800"},
			...
			{Name: "confirm_unattended", Type: "boolean", Description: "显式确认无人值守 Agent。只在真正执行命令的 kind=agent 任务上必填；reply_text 聊天回复不需要"},
			{Name: "reply_text", Type: "string", Description: "要直接发进当前聊天的原文。用户要在当前聊天框发一句话时必填；到点直接发送，不另起 Agent"},
		},
		Actions: []ActionSpec{
			{Name: "plan", Description: "只解析计划，不落盘", AnyOf: [][]string{{"text", "task", "interval_sec"}}},
			{Name: "add", Description: "创建定时任务；只在用户明确要求创建时使用。周期用 interval_sec，正文用 task", Required: []string{"confirm_create"}, AnyOf: [][]string{{"text", "task", "interval_sec"}}},
			{Name: "list", Description: "列出任务"},
			{Name: "show", Description: "查看一个任务", Required: []string{"id"}},
			{Name: "edit", Description: "修改正文、名称、周期或时限", Required: []string{"id"}},
			{Name: "pause", Required: []string{"id"}}, {Name: "cancel", Description: "取消后续触发", Required: []string{"id"}}, {Name: "resume", Required: []string{"id"}},
			{Name: "reset", Description: "清除失败并重新安排；不撤销已经做过的事", Required: []string{"id"}},
		...
		Examples: []map[string]string{{"action": "list"}, {"action": "add", "task": "每分钟在当前聊天框发一句 hello", "repeat": "interval", "interval_sec": "60", "reply_text": "hello", "confirm_create": "true"}},
	},
	"skill_market": {
		Args: []ArgSpec{
